          ...
          return (
            <div key={s.name} className="service-card" onClick={() => onServiceClick(s.name)}>
              ...
              <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.5rem' }}>
                <span className="ticket-count">{s.tickets.toLocaleString()}</span>
              ...
              {s.tickets > 0 && (
                <>
                  <div className="mini-trend">
                    {trend.map((t, i) => (
                      <div key={i} className="mini-bar" style={{ height: `${Math.max(2, (t.count / maxTrend) * 28)}px` }} title={`${t.year}: ${t.count}`} />
                    ))}
                  </div>
